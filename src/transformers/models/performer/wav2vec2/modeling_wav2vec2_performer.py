...
from .configuration_wav2vec2_performer import Wav2Vec2PerformerConfig
from ..modeling_performer_attention import PerformerAttention

logger = logging.get_logger(__name__)

_CONFIG_FOR_DOC = "Wav2Vec2PerformerConfig"
...
class Wav2Vec2Attention(nn.Module):
    ...
    def forward(
        ...
        performer_shape_proj = (bsz, tgt_len, -1, 1)
        attention_mask_shape_proj = (bsz, -1)  #bsz, 1, seq_len, 1
...
class Wav2Vec2PerformerPreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = Wav2Vec2PerformerConfig
...
class Wav2Vec2PerformerModel(Wav2Vec2PerformerPreTrainedModel):
    def __init__(self, config):
...
class Wav2Vec2PerformerForMaskedLM(Wav2Vec2PerformerPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        ...
        self.wav2vec2 = Wav2Vec2PerformerModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class Wav2Vec2PerformerForCTC(Wav2Vec2PerformerPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
